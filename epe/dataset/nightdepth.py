...
def transform_labels(original_label_map):
    ...
    for k, v in id2label.items():
        label_map[:, :, k] = (np.isin(original_label_map, v)).astype(np.long)
...
class NightDepth(RobustlyLabeledDataset):
    ...
    def __getitem__(self, index):
        ...
        img2 = img2.resize((img.shape[2], img.shape[1]), resample=PIL.Image.NEAREST)
        robust_labels = np.array(img2)
